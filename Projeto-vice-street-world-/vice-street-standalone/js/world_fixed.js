// Sistema de mundo e ambiente urbano corrigido
class WorldManager {
    constructor(scene) {
        this.scene = scene;
        this.objects = [];
    ...
    addNeonSigns() {
        ...
        for (let i = 0; i < positions.length; i++) {
            ...
            const signLight = new THREE.PointLight(sign.color, 1, 10, 2);
            signLight.position.set(pos.x, pos.y, pos.z + 0.5);
    ...
    setupMission1Objects() {
        try {
            // Adicionar moto de gangue (Harley)
            this.createMotorcycle();
            
            // Adicionar Magnum .357
            this.createMagnum();
            
            // Adicionar NPCs
            this.createNPCs();
        } catch (error) {
            console.warn("Erro ao configurar objetos da Missão 1:", error);
        }
    }
    
    createMotorcycle() {
        try {
            // Criar moto simplificada (usando formas geométricas básicas)
            ...
            // Roda dianteira
            const frontWheelGeometry = new THREE.CylinderGeometry(0.8, 0.8, 0.3, 16);
            ...
            handlebar.position.set(0, 1.5, 1.8);
            ...
            // Posicionar moto na cena
            motorcycle.position.set(15, 0, 15);
            ...
            // Configurar como objeto interativo
            motorcycle.userData = {
                interactable: true,
                type: 'motorcycle',
            ...
            // Adicionar à lista de objetos interativos
            if (motorcycle) {
                this.interactableObjects.push(motorcycle);
            }
        } catch (error) {
            console.warn("Erro ao criar moto:", error);
        }
    }
    
    createMagnum() {
        try {
            // Criar Magnum .357 simplificada
            ...
            // Tambor
            const cylinderGeometry = new THREE.CylinderGeometry(0.15, 0.15, 0.2, 8);
            ...
            // Posicionar arma na cena
            magnum.position.set(-12, 1, -18);
            magnum.rotation.set(0, Math.PI / 3, 0);
            magnum.scale.set(2, 2, 2);
            ...
            console.warn("Erro ao criar Magnum:", error);
        }
    }
    ...
    createNPCs() {
        try {
            // Criar NPCs simples
            const npcPositions = [
                { x: 5, z: 5, name: 'Carlos', dialog: 'Ei, você deve ser o Vic. Ouvi falar de você. Dizem que tem uma moto de gangue por aqui em algum lugar.' },
                { x: -8, z: 10, name: 'Maria', dialog: 'Cuidado com as ruas à noite. Você vai precisar de proteção. Procure por uma arma.' }
            ];
            ...
            npcPositions.forEach(npcData => {
                ...
                const bodyMaterial = new THREE.MeshStandardMaterial({
                    color: Math.random() > 0.5 ? 0x0066ff : 0xff6600,
                    roughness: 0.8,
                    metalness: 0.2
                });
                const body = new THREE.Mesh(bodyGeometry, bodyMaterial);
                ...
                // Posicionar NPC
                npc.position.set(npcData.x, 0, npcData.z);
                ...
                npc.userData = {
                    ...
                    message: `Pressione E para falar com ${npcData.name}`
                };
                
                // Adicionar à lista de objetos interativos
                if (npc) {
                    this.interactableObjects.push(npc);
                }
            });
        } catch (error) {
            console.warn("Erro ao criar NPCs:", error);
        }
    }
    
    setupWeather() {
        // Configurar efeitos climáticos com base no clima definido
    ...
    setupRain() {
        try {
            ...
            // Adicionar à lista de objetos para atualização
            this.objects.push(rain);
            
            // Função para animar a chuva
            this.animateRain = function(delta) {
                ...
                for (let i = 0; i < positions.length; i += 3) {
                    ...
                    // Resetar posição quando atingir o chão
                    if (positions[i + 1] < 0) {
                        positions[i + 1] = 100;
                    }
                }
            ...
            console.warn("Erro ao configurar chuva:", error);
        }
    }
    
    update(delta) {
        // Atualizar efeitos climáticos
        if (this.weather === 'rain' && this.animateRain) {
            this.animateRain(delta);
        }
        
        // Atualizar luzes de neon (piscando suavemente)
        this.updateNeonLights(delta);
    }
    
    updateNeonLights(delta) {
        // Fazer algumas luzes de neon piscarem suavemente
        const time = Date.now() * 0.001;
        
        this.lights.forEach((light, index) => {
            if (light.type === 'PointLight' && light.color.r > 0.5) {
                // Apenas luzes coloridas (neon)
                const intensity = 0.5 + Math.sin(time + index * 0.5) * 0.3;
    ...
    getInteractableObjects() {
        // Filtrar objetos nulos ou indefinidos
        return this.interactableObjects.filter(obj => obj !== null && obj !== undefined);
    }
